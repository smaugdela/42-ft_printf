...
/*   Updated: 2021/08/13 15:12:26 by smagdela         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static int	ft_strlen_charset(char *str, char *charset)
{
	int	len;

	len = 0;
	while(ft_is_in_charset(*str, charset) == 0 && *str)
	{
		++len;
		++str;
	}
...
static int	ft_dispatcher(const char *specifier, int spec_len, va_list arguments)
{
	if (ft_is_in_charset(specifier[spec_len - 1], "c%"))
		return (ft_print_cpercent(specifier, va_arg(arguments, char)));
